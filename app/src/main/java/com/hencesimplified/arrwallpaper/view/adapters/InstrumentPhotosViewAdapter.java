...
import com.hencesimplified.arrwallpaper.view.fragments.InstrumentsFragmentDirections;

import java.util.List;

public class InstrumentPhotosViewAdapter extends RecyclerView.Adapter<InstrumentPhotosViewAdapter.PhotosViewHolder> {

    private List<PhotoData> photosList;

    public InstrumentPhotosViewAdapter(List<PhotoData> photosList) {
    ...
    public void onBindViewHolder(@NonNull PhotosViewHolder holder, final int position) {
        ...
        photoCardView.setOnClickListener(view -> {
            ...
            InstrumentsFragmentDirections.InstrumentToPhotos instrumentToPhotos = InstrumentsFragmentDirections.instrumentToPhotos();
            instrumentToPhotos.setPhotoUrl(img);
            Navigation.findNavController(photoCardView).navigate(instrumentToPhotos);
        });
    }
