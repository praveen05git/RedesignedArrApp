...
import com.hencesimplified.arrwallpaper.view.fragments.FamilyFragmentDirections;

import java.util.List;

public class FamilyPhotosViewAdapter extends RecyclerView.Adapter<FamilyPhotosViewAdapter.PhotosViewHolder> {

    private List<PhotoData> photosList;

    public FamilyPhotosViewAdapter(List<PhotoData> photosList) {
    ...
    public void onBindViewHolder(@NonNull PhotosViewHolder holder, final int position) {
        ...
        photoCardView.setOnClickListener(view -> {
            ...
            FamilyFragmentDirections.FamilyToPhotos familyToPhotos = FamilyFragmentDirections.familyToPhotos();
            familyToPhotos.setPhotoUrl(img);
            Navigation.findNavController(photoCardView).navigate(familyToPhotos);
        });
    }
